...
> Legacy note (only if uploads are under webroot in development): The recommended production setup stores all uploads outside the webroot and serves them exclusively via PHP streaming endpoints. In that model, web server rules like `.htaccess` are not relied upon for protection. If you temporarily keep uploads under the webroot (e.g., during local development), use the following as an additional safeguard:
```apache
# uploads/.htaccess
php_flag engine off
...
### 🗂️ **Secure Storage Model (Uploads & Certificates)**

#### Overview
- All uploaded documents and generated certificates are stored outside the webroot using a centralized storage base.
- Access to files is mediated through secure PHP endpoints; direct URL access to raw files is not possible.

#### Storage Paths
- Default base path: sibling folder to the project root (e.g., `ipmo_storage`).
- Override with environment variable `STORAGE_PATH` for production deployments (recommended: a path outside the webroot).
- Helper: `storage_path($subdir)` in `config.php` resolves paths like:
    - `storage_path('uploads')`
    - `storage_path('certificates')`

#### Endpoints (Streaming Access)
- User endpoint: `User/Afterlogin/download_document.php`
    - Validates session and ownership before streaming
    - Returns correct content-type and headers
- Admin endpoint: `admin/stream_document.php`
    - Requires admin session/role
    - Streams files from secure storage

#### Certificates
- Generated by `includes/certificate_generator.php` into `storage_path('certificates')`.
- File URLs in the UI point to the streaming endpoints, not to filesystem paths.

#### Production Recommendations
- Set `STORAGE_PATH` to an absolute path outside webroot (e.g., `D:\\ipmo_storage` on Windows, `/var/lib/ipmo_storage` on Linux).
- Ensure backup/restore procedures include the storage directory.
- Never expose the storage directory via your web server.

#### Verification
- `verify_security_updates.php` checks that:
    - The storage base exists and is writable
    - It is outside the webroot (recommended)
    - Legacy `uploads/` under webroot is not used
    - Streaming endpoints are present
...
> Note: In development, a CSP reporting endpoint is configured to avoid persistent browser console warnings while still capturing violations. In production, CSP is enforced and also configured to report violations.
...
function validateSecurityConfig() {
    ...
    $storagePath = Environment::get('STORAGE_PATH');
    if (!$storagePath) {
        $issues[] = 'STORAGE_PATH not configured (using default fallback). In production set an absolute path outside webroot.';
    }
...
# Secure storage: set an absolute path OUTSIDE the webroot in production.
# Windows example
STORAGE_PATH=D:\\ipmo_storage
# Linux example
# STORAGE_PATH=/var/lib/ipmo_storage
```
