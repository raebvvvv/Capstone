...
if($upd->execute()) {
    if(function_exists('log_event')) { log_event('USER_PROFILE_UPDATE', 'Profile updated', ['uid'=>$user_id]); }
    // Refresh values in session if stored
    $_SESSION['username'] = $name;
    $_SESSION['email'] = $email;
    echo json_encode(['success'=>true,'name'=>$name,'email'=>$email]);
} else {
    if(function_exists('log_event')) { log_event('USER_PROFILE_UPDATE_FAIL', 'DB update failed', ['err'=>$upd->error]); }
