...
try{
  ...
  $ids = [];
  if(isset($_POST['ids'])){ $raw = $_POST['ids']; if(is_string($raw)) $raw = json_decode($raw, true); if(is_array($raw)) $ids = array_map('intval', $raw); }
  ...
  $st = $pdo->prepare($sql); $st->execute($params); $deleted = $st->rowCount();
  $st2 = $pdo->prepare('SELECT COUNT(*) AS cnt FROM user_notifications WHERE user_id = ? AND is_read = 0 AND deleted_at IS NULL'); $st2->execute([$user_id]); $row = $st2->fetch(PDO::FETCH_ASSOC); $count = $row ? (int)$row['cnt'] : 0;
