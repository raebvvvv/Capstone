  ...
  <!-- Application Overview Section -->
    ...
    document.getElementById('changePasswordForm')?.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const res = await fetch('../change_password.php', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': csrf }, body: JSON.stringify({ current_password: cur, new_password: pass }) });
            let data; const text = await res.text(); try { data = JSON.parse(text); } catch { data = { success:false, error:text.trim() || 'Unexpected response' }; }
            if (!res.ok) { return showAlert(data.error || `Error ${res.status}`); }
            if (data.success) { showAlert('Password updated successfully.', 'success'); document.getElementById('currentPassword').value=''; document.getElementById('newPassword').value=''; document.getElementById('confirmPassword').value=''; }
            else { showAlert(data.error || 'Failed to update password.'); }
        } catch (err) { showAlert('Network error. Please try again.'); }
    });
    </script>
</body>
</html>
